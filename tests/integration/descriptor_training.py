...
from aef.train.descriptor import process_batch_homographic_descriptor
...
class MockModel(torch.nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        features = torch.eye(H * W).reshape(-1, H * W, H, W).to(x.device)
        if torch.abs(torch.mean(x)) < 1e-3:
            features = features = kornia.geometry.transform.warp_perspective(
                features,
                self.gt_transform.to(x.device),
                dsize=(H, W)
            )
        return features
...
def assert_loss(pred, gt):
    fpr_val = fpr()(pred, gt)
    torch.testing.assert_close(fpr_val, torch.Tensor((0.0,)).to(pred.device))
...
def test_descriptor_training_homographic(transform, monkeypatch):
    ...
    train_func(process_batch_homographic_descriptor)(model, dataset, dataset, DEFAULT_CFG)
