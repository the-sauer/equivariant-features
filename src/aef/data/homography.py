...
from ..geometry import linearize_homography
...
#   5: per-patch log-polar validity mask added to the cached state; bumped so pre-v5
#      caches (which lack the tensor) are rebuilt.
CACHE_VERSION = 5
...
# boards, keypoints or extracted patches (patch_size, supersample, the compositing/
# garbage settings, augmentation, ...) is covered.
_CACHE_KEY_EXCLUDE = {
    "data_dir", "cache_dir", "cache_path", "extraction_batch_size", "sift_batch_size",
}

# Constructor params that were part of the key before they were removed from the code
# (cartesian patches, lens distortion, the validity masks, the scale-band / keypoint-cap
# / detector-jitter knobs — none of which any shipped config used). `effective_params`
# can no longer produce them, so they are pinned here at the values the surviving
# configs ran with: without this every already-prepared dataset under `cache_dir` would
# key differently and be rebuilt from scratch (board render + SIFT + patch extraction)
# for a code change that provably cannot have altered its contents. A caller that still
# passes one of these names overrides the pin. Drop an entry — and bump CACHE_VERSION —
# only when you *want* those caches rebuilt.
_CACHE_KEY_REMOVED = {
    "patch_type": "logpolar",
    "patch_scale_factors": [16.0, 64.0, 128.0],
    "precompute_masks": True,
    "scale_quantile_range": None,
    "scale_range": None,
    "max_keypoints": None,
    "subsample_seed": 0,
    "keypoint_jitter": 0.0,
...
def dataset_cache_key(params: dict) -> str:
    ...
    payload = {"__cache_version__": CACHE_VERSION, **_CACHE_KEY_REMOVED}
...
def blob_normalizations(homographies, coords, device):
    ...
    SVD) so that the elliptical blob maps to an isotropic one.
    ...
    size. ``scales`` already carries the warp's ``sqrt(det J)`` and the extractor
    divides by it, so a factor that also removed the size would remove it twice and
...
class HomographyData(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        supersample=3,  # sub-taps per output pixel per axis, area-averaged
        ...
        **_,
        ...
        # __getitem__). Their identity-view patch is a bland clean-board sample; only
        # their warped views carry real background content.
        ...
        if self._board_masks is not None and self._garbage_fraction > 0:
            with torch.no_grad():
                ...
                # We pool SIFT background detections (preferred) and random
                # background points (top-up), then pick exactly this many.
    ...
    def _load_cache(self, path):
        # Only the keys this class still knows about are restored: a cache written by an
        # older revision carries extra entries (per-view lens params, the cartesian patch
        # settings, the per-patch validity masks) whose features no longer exist, and
        # setting them would resurrect dead attributes rather than fail loudly.
        state = torch.load(path, map_location="cpu", weights_only=False)
        for k in self._CACHE_META:
            if k in state["meta"]:
                setattr(self, k, state["meta"][k])
        for k in self._CACHE_TENSORS:
            if k in state["tensors"]:
                setattr(self, k, state["tensors"][k])
        self._board_masks = None
    ...
    def compute_patches(self):
        ...
        if self.in_memory:
            ...
            # bereits die Skala).
            print("Pre-extracting logpolar patches into memory...")
            # Allokiere Speicher für alle Patches (N, 1, H, W)
    ...
    def __getitem__(self, index):
        ...
        if self.patches_available:
            ...
            # rendering, i.e. this dataset's `is_pdf` patch — the LOSS-side label the
            # proxy-anchored objectives key on (`ProxyAnchoredSupCon` contrasts PDF
